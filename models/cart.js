...
class Cart {
    ...
    getListData({uid},callback){
        pool.getConnection(function(err,connection){
            if(err) throw err
            connection.query(`select * from cart,product where uid=${uid} and product.pid=cart.pid`,function(err,results){
                if(err) throw err
                callback(results)
    ...
    changeNumber({cart_id,number},callback){
        ...
        pool.getConnection(function(err,connection){
            ...
            console.log(`update cart set number=${number} where cart_id=${cart_id}`)
